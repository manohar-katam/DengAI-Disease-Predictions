...
#Read the dataset
dengue_labels_train <- read.csv("/Users/shivapodugu/Downloads/dengue_features_train.csv")
dengue_features_train <- read.csv("/Users/shivapodugu/Downloads/dengue_labels_train.csv")
...
sj.train.train <- na.omit(sj.train.train)
iq.train.train <- na.omit(iq.train.train)

library(e1071)
set.seed(412)

# sj
sj.SVM.train =svm(total_cases~ ., data=sj.train.train, gamma=0.08, epsilon=0.1, cost=0.05)
sj.test.pred <- predict(sj.SVM.train,sj.train.test[,-20])
library(MLmetrics)
MAE(sj.test.pred,sj.train.test[,20])
RMSE(sj.test.pred,sj.train.test[,20])
R2_Score(sj.test.pred,sj.train.test[,20])

#MAE: 18.98571 gamma=0.08, epsilon=0.1, cost=0.09
#MAE: 18.84939 gamma=0.08, epsilon=0.1, cost=0.05 

sj.train.test$pred <- as.numeric(sj.test.pred)
...
iq.SVM.train =svm(total_cases~ ., data=iq.train.train, gamma=0.08, epsilon=0.1, cost=0.05)
iq.test.pred <- predict(iq.SVM.train,iq.train.test[,-20])
library(MLmetrics)
MAE(iq.test.pred,iq.train.test[,20])
RMSE(iq.test.pred,iq.train.test[,20])
R2_Score(iq.test.pred,iq.train.test[,20])

iq.train.test$pred <- as.numeric(iq.test.pred)
