...
test <- read.csv('C:/Users/manohar/Documents/R/DengAI/dengue_features_test.csv')
...
sj.test <- subset(test, city == "sj")
iq.test <- subset(test, city == "iq")

for (i in attributes){
  sj.test[i] <- na.locf(sj.test[i], fromLast = TRUE)
  sj.test[i] <- as.numeric(unlist(sj.test[i]))
  iq.test[i] <- na.locf(iq.test[i], fromLast = TRUE)
  iq.test[i] <- as.numeric(unlist(iq.test[i]))
}

sj.test <- sj.test[, !(names(sj.test) %in% for.removal)]
iq.test <- iq.test[, !(names(iq.test) %in% for.removal)]

sj.test[c('total_cases','lag 1','lag 2','lag 3','lag 4','lag 5')] <- NA
iq.test[c('total_cases','lag 1','lag 2','lag 3','lag 4','lag 5')] <- NA

sj.test[c('total_cases','lag 1','lag 2','lag 3','lag 4','lag 5')] <- 
  as.integer(unlist(sj.test[c('total_cases','lag 1','lag 2','lag 3','lag 4','lag 5')]))

iq.test[c('total_cases','lag 1','lag 2','lag 3','lag 4','lag 5')] <- 
  as.integer(unlist(iq.test[c('total_cases','lag 1','lag 2','lag 3','lag 4','lag 5')]))

sj.test.frame <- as.h2o(sj.test, destination_frame = "sj.test")
iq.test.frame <- as.h2o(iq.test, destination_frame = "iq.test")

predictions.rf <- data.frame(Prediction = double())

for (i in 1:nrow(sj.test)){
  sj.train = rbind(sj.train, sj.test[i,])  
  sj.train[nrow(sj.train),'lag 1'] <- sj.train[nrow(sj.train)-1, 'total_cases']
  sj.train[nrow(sj.train),'lag 2'] <- sj.train[nrow(sj.train)-2, 'total_cases']
  sj.train[nrow(sj.train),'lag 3'] <- sj.train[nrow(sj.train)-3, 'total_cases']
  sj.train[nrow(sj.train),'lag 4'] <- sj.train[nrow(sj.train)-4, 'total_cases']
  sj.train[nrow(sj.train),'lag 5'] <- sj.train[nrow(sj.train)-5, 'total_cases']
  pred_frame <- as.h2o(sj.train[nrow(sj.train), ], destination_frame = "pred_frame")
  
  pred <- as.data.frame(h2o.predict(sj.gb.train,pred_frame))
  sj.train[nrow(sj.train),'total_cases'] <- pred$predict
  predictions.rf <- rbind(predictions.rf,pred)
}

for (i in 1:nrow(iq.test)){
  iq.train = rbind(iq.train, iq.test[i,])  
  iq.train[nrow(iq.train),'lag 1'] <- iq.train[nrow(iq.train)-1, 'total_cases']
  iq.train[nrow(iq.train),'lag 2'] <- iq.train[nrow(iq.train)-2, 'total_cases']
  iq.train[nrow(iq.train),'lag 3'] <- iq.train[nrow(iq.train)-3, 'total_cases']
  iq.train[nrow(iq.train),'lag 4'] <- iq.train[nrow(iq.train)-4, 'total_cases']
  iq.train[nrow(iq.train),'lag 5'] <- iq.train[nrow(iq.train)-5, 'total_cases']
  pred_frame <- as.h2o(iq.train[nrow(iq.train), ], destination_frame = "pred_frame")
  
  pred <- as.data.frame(h2o.predict(iq.gb.train,pred_frame))
  iq.train[nrow(iq.train),'total_cases'] <- pred$predict
  predictions.rf <- rbind(predictions.rf,pred)
}

write.csv(round(predictions.rf,0), file = "C:\\Users\\manohar\\Documents\\R\\DengAI\\dengai_pred_extra.csv")
